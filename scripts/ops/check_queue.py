...
try:
    ...
    unclaimed = db.query(func.count(EmailQueue.id)).filter_by(account_id=account.id, claimed_by=None).scalar()
    claimed = db.query(func.count(EmailQueue.id)).filter(
        EmailQueue.account_id == account.id,
        EmailQueue.claimed_by.isnot(None)
    ).scalar()
